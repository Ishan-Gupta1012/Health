...
export function FeaturesSection() {
  ...
  const renderProtectedButton = (label: string, feature: string, href: string) => {
    const isEnabled = !isUserLoading && !!user;

    const button = (
      <Button asChild variant="outline" className="w-full mt-auto bg-white/30 hover:bg-white/50 text-foreground neon-shadow-primary-hover" disabled={!isEnabled}>
        <Link href={isEnabled ? href : '#'}>{label}</Link>
      </Button>
    );
    ...
    <section id="home" className="w-full py-12 md:py-24 lg:py-32">
      <div className="container px-4 md:px-6">
        ...
        <div className="mx-auto mt-12 grid max-w-5xl grid-cols-1 gap-8 sm:grid-cols-2">
          ...
          >
            {renderProtectedButton('Find a Doctor', 'Doctor Finder', '/doctor-finder')}
          </FeatureCard>
          <FeatureCard
          ...
          >
            {renderProtectedButton('Set Reminder', 'Medicine Reminders', '/medicine-reminders')}
          </FeatureCard>
          <FeatureCard
          ...
          >
            {renderProtectedButton('Share Records', 'Medical Record Sharing', '/medical-record-sharing')}
          </FeatureCard>
        </div>
      </div>
    </section>
